...
const Table = ({ teachers, lectures }) => {
  ...
  const [timeInputs, setTimeInputs] = useState(Array(teachers).fill(''));
  ...
  const handleDaySelect = (dayIndex, row, col) => {
    const updatedSelectedDays = selectedDays.map((rowDays, rowIndex) =>
      rowDays.map((days, colIndex) => {
        if (rowIndex === row && colIndex === col) {
          return days.map((selected, index) => index === dayIndex ? !selected : selected);
        }
        return days;
      })
    );
  ...
  const handleTimeChange = (i, event) => {
    ...
    newTimeInputs[i] = event.target.value;
                        ...
                        <div className='mt-2'>
                          {weekdays.map((day, dayIndex) => (
                            <span
                              ...
                              className={`cursor-pointer px-2 py-1 text-sm rounded-full mr-1 ${selectedDays[i][j][dayIndex] ? 'bg-gray-800 text-white' : 'bg-gray-200'
                                }`}
                            >
                              {day}
                            </span>
      ...
      {teachers > 0 && lectures > 0 && (
        <div className='print-tables'>
          ...
          <div className='print-table-container'>
            {/* Map through the columns and create separate tables for each set of 5 columns */}
            {Array.from({ length: Math.ceil(lectures / 5) }, (_, tableIndex) => (
              <table key={tableIndex} className='print-table'>
                <thead>
                  <tr>
                    <th>Classes</th>
                    {Array.from({ length: Math.min(5, lectures - tableIndex * 5) }, (_, i) => (
                      <th key={i}>Class {tableIndex * 5 + i + 1}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {Array.from({ length: teachers }, (_, i) => (
                    <tr key={i}>
                      ...
                      {Array.from({ length: Math.min(5, lectures - tableIndex * 5) }, (_, j) => (
                        <td key={j}>
                          <div className='flex flex-col'>
                            <select
                              value={selectedTeachers[i][tableIndex * 5 + j]}
                              className='w-full p-2 rounded-lg border-none text-sm'
                              style={{
                                backgroundColor: rowDuplicateTeachers[i].has(selectedTeachers[i][tableIndex * 5 + j]) ? 'red' : 'white'
                              }}
                            >
                              <option value=''>Select Teacher</option>
                              {teacherNames.map((teacher, index) => (
                            ...
                            <input
                              type='text'
                              value={subjects[i][tableIndex * 5 + j]}
                              placeholder='Enter subject'
                            ...
                            <div className='mt-2'>
                              {weekdays.map((day, dayIndex) => (
                                <span
                                  key={dayIndex}
                                  className={`px-2 py-1 text-sm rounded-sm mr-1 ${selectedDays[i][tableIndex * 5 + j][dayIndex] ? 'bg-blue-700 text-slate-950' : ' bg-red-700 text-slate-100'
                                    }`}
                                >
                                  {day}
                                </span>
